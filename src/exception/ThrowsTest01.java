...
public class ThrowsTest01 {
	...
	public static void main(String[] args) {
		...
		//      결정하는 것이므로 반드시 예외가 발생하는 곳, 혹은 예외 발생하는 메소드를 호출하는 곳 둘 중 한곳에서는 반드시 try~catch로 처리해야한다.
		try {
			obj.test2("test1.txt");
			}catch(FileNotFoundException e) {
				System.out.println("파일을 찾을 수 없습니다. 파일선택 대화상자를 다시 실행해서 파일을 직접 선택하도록 처리---->A방법");
		}
		
		try {
			obj.test2("test2.txt");
			}catch(FileNotFoundException e) {
				System.out.println("파일을 찾을 수 없습니다. 파일선택 대화상자를 다시 실행해서 파일을 직접 선택하도록 처리---->B방법");
			}
	}
}