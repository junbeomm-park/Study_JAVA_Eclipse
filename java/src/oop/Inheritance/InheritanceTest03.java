...
// 2. 모든 클래스의 최상위 클래스는 java.lang.Object 클래스 이다.
...
class SubA extends SuperA{   //1. SubA에는 기본생성자가 생략되어 있다.
	...
	public SubA() {          //2. 모든 생성자의 첫 번째 문장에는 부모의 기본생성자 호출문이 생략되어 있다.
		 super();            //3. 부모클래스에서는 매개변수가 있는 생성자가 이미 정의되어 있으므로 컴파일러가 기본생성자를 추가해주지 않는다.
		                     ...
		                     //SubA가 name을 갖고 있지 않아도 name 변수를 전달받을 수 있도록 추가해야한다.
	}
...
public class InheritanceTest03 {
	public static void main(String[] args) {
		...
		//따라서 하위클래스를 사용하는 경우 하위클래스가 직접적으로 멤버변수를 갖고 있지 않다고 하더라도 모든 값을 하위클래스에서 받을 수 있도록 생성자를
		//정의해야한다.
		SubA obj2 = new SubA("장동건",20,"인천");
		obj.display(); // obj객체는 맴버의 변수 값이 아무것도 셋팅되어 있지 않으므로 모두 결과가 값이 출력되지 않는다.
